...
#define UPPER_LIMIT 300.0  // jump up threshold
#define LOWER_LIMIT -500.0 // jump down threshold

#define FAST_LIMIT 2.0 // jump up threshold
#define SLOW_LIMIT 0.0 // jump down threshold
...
void loop() {
  ...
  // check jump up
//  if ((gyroY > UPPER_LIMIT) && (accX > FAST_LIMIT)) {
  if (gyroY > UPPER_LIMIT) {
    upcount++;
  }
  // jump down and count
  if (gyroY < LOWER_LIMIT && upcount > 0)  { //ここでaccYは問わない
    upcount = 0;
    counter++;
  }
